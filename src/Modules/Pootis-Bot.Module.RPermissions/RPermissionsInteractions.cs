...
[global::Discord.Interactions.Group("perm", "Commands related to permissions")]
public class RPermissionsInteractions : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task PermGet([Remainder] string command)
    {
        CommandSearchResult result = FindSlashCommand(command);
        if (result.IsSuccess)
        {
    ...
    private CommandSearchResult FindSlashCommand(string command)
    {
        if (string.IsNullOrWhiteSpace(command))
            return CommandSearchResult.FromError("You must input a valid command name!", CommandError.UnknownCommand, null);

        int count = interactionService.SlashCommands.Count(x => x.Name == command);
        if (count != 1) 
            return CommandSearchResult.FromError("No command was found!", CommandError.Unsuccessful, null);
    ...
    private async Task RemovePermission(SlashCommandInfo commandInfo, IGuild guild, [MaybeNull] IRole role)
    {
        RPermissionServer server = config.GetOrCreateServer(guild.Id);
